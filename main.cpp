...
unsigned int triangle_count = 11000;
...
LRESULT w_proc(HWND window, UINT msg, WPARAM wparam, LPARAM lparam) {
    switch (msg) {
        case WM_SIZE: {
            width = std::max((WORD)1, LOWORD(lparam));
            height = std::max((WORD)1, HIWORD(lparam));
            ...
            if (device) {
                ...
                misc = { width, height, triangle_count };

                ctx->UpdateSubresource(buf5, 0, nullptr, (const void*)misc.data(), 0, 0);
            }
...
int WinMain(HINSTANCE h_inst, HINSTANCE p_inst, LPSTR lpcmdln, int n_cmd_show) {
    ...
    misc = { width, height, triangle_count }; // Screen width, screen height, triangle count
    ...
    ID3D11Buffer* buf3;
    hr = shader.create_D3D11_buffer(&buf3, (const void*)positions, sizeof(float) * 9 * triangle_count, sizeof(float), D3D11_BIND_SHADER_RESOURCE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, 0, D3D11_USAGE_DEFAULT);
    if (FAILED(hr)) { return 5; }

    ID3D11Buffer* buf4;
    hr = shader.create_D3D11_buffer(&buf4, (const void*)colors, sizeof(float) * 13 * triangle_count, sizeof(float), D3D11_BIND_SHADER_RESOURCE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, 0, D3D11_USAGE_DEFAULT);
    if (FAILED(hr)) { return 6; }

    hr = shader.create_D3D11_buffer(&buf5, (const void*)misc.data(), sizeof(unsigned int) * 3, sizeof(unsigned int), D3D11_BIND_SHADER_RESOURCE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, 0, D3D11_USAGE_DEFAULT);
    if (FAILED(hr)) { return 7; }

    ID3D11Buffer* buf6;
    hr = shader.create_D3D11_buffer(&buf6, (const void*)tri_meta.data(), sizeof(float) * 5 * triangle_count, sizeof(float), D3D11_BIND_SHADER_RESOURCE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, 0, D3D11_USAGE_DEFAULT);
    if (FAILED(hr)) { return 7; }
    ...
    hr = shader.create_SRV(buf3, &SRV1, 9 * triangle_count, D3D11_SRV_DIMENSION_BUFFER);
    if (FAILED(hr)) { std::cout << "Error creating D3D11 SRV\n" << std::hex << hr << "\n"; return -5; }

    ID3D11ShaderResourceView* SRV2 = nullptr;
    hr = shader.create_SRV(buf4, &SRV2, 13 * triangle_count, D3D11_SRV_DIMENSION_BUFFER);
    if (FAILED(hr)) { std::cout << "Error creating D3D11 SRV\n" << std::hex << hr << "\n"; return -5; }

    ID3D11ShaderResourceView* SRV3 = nullptr;
    hr = shader.create_SRV(buf5, &SRV3, 3, D3D11_SRV_DIMENSION_BUFFER);
    if (FAILED(hr)) { std::cout << "Error creating D3D11 SRV\n" << std::hex << hr << "\n"; return -5; }

    ID3D11ShaderResourceView* SRV4 = nullptr;
    hr = shader.create_SRV(buf6, &SRV4, 5 * triangle_count, D3D11_SRV_DIMENSION_BUFFER);
    if (FAILED(hr)) { std::cout << "Error creating D3D11 SRV\n" << std::hex << hr << "\n"; return -5; }
    ...
    while (running) {
        ...
        for (unsigned int i = 0; i < triangle_count; i++) {
            unsigned int minx = std::min(positions[i * 9], std::min(positions[i * 9 + 3], positions[i * 9 + 6]));
            unsigned int miny = std::min(positions[i * 9 + 1], std::min(positions[i * 9 + 4], positions[i * 9 + 7]));
            unsigned int maxx = std::max(positions[i * 9], std::max(positions[i * 9 + 3], positions[i * 9 + 6]));
            unsigned int maxy = std::max(positions[i * 9 + 1], std::max(positions[i * 9 + 4], positions[i * 9 + 7]));
        ...
        ctx->UpdateSubresource(buf6, 0, nullptr, tri_meta.data(), 0, 0);
        ctx->Dispatch(triangle_count, 1, 1);
        ctx->Flush();

        retrieve = shader.retrieve_D3D11_process<unsigned int>(CPU_buf, buf1);
        if (retrieve) memcpy(screen_mem, retrieve, sizeof(unsigned int) * width * height);

        tri_meta.clear();
        tri_meta.reserve(triangle_count * 5);
