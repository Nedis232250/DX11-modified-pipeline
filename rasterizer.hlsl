...
RWStructuredBuffer<uint> screen_buf : register(u0);
RWStructuredBuffer<float> z_buf : register(u1);
RWStructuredBuffer<Fragment> fragment_buffer : register(u2);
...
void main(uint3 threadID : SV_DispatchThreadID, uint3 groupID : SV_GroupID) {
    ...
    if (i < (misc[2] * 144)) {
        ...
        float3 as = float3(c[t * 13 + 3], c[t * 13 + 7], c[t * 13 + 11]);
        float l1, l2, l3;
        uint mode = c[t * 13 + 12];
            ...
            for (uint x = minx; x < maxx; x++) {
                for (uint y = miny; y < maxy; y++) {
                    ...
                    if (l1 >= 0.0f && l2 >= 0.0f && l3 >= 0.0f) {
                        float3 rgb_src = l1 * c1 + l2 * c2 + l3 * c3;
                        float r = saturate(rgb_src.r);
                        float g = saturate(rgb_src.g);
                        float b = saturate(rgb_src.b);

                        screen_buf[1540 * y + x] = 0xFF00FF;
                        
                        if ((l1 * zs.x + l2 * zs.y + l3 * zs.z) < fragment_buffer[1540 * y + x].depth1) {
                            Fragment f = {
                                { r, g, b, l1 * as[0] + l2 * as[1] + l3 * as[2] }, fragment_buffer[1540 * y + x].color2,
                                (l1 * zs.x + l2 * zs.y + l3 * zs.z), fragment_buffer[1540 * y + x].depth2
                            };
                        
                            fragment_buffer[1540 * y + x] = f;
                        } else if ((l1 * zs.x + l2 * zs.y + l3 * zs.z) < fragment_buffer[1540 * y + x].depth2) {
                            Fragment f = {
                                fragment_buffer[1540 * y + x].color1, { r, g, b, l1 * as[0] + l2 * as[1] + l3 * as[2] },
                                fragment_buffer[1540 * y + x].depth1, (l1 * zs.x + l2 * zs.y + l3 * zs.z)
                            };
